...
Le code applicatif de l’**API** se trouve dans [`backend/`](../backend/) (NestJS + Prisma). Le dossier [`docs/`](.) rassemble les livrables rédigés (expression du besoin, cahier des charges, architecture, estimation, planning).
...
| ORM (API) | **Prisma** (retenu pour l’implémentation dans `backend/`) |
